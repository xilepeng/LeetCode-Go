...
func QuickSort(A []int, start, end int) {
	...
	x := A[start+(end-start)>>1] // x := A[(start+end)/2],用j划分递归子区间
...
// Your memory usage beats 5.02 % of golang submissions (13.6 MB)
func quickSort(A []int, low, high int) {
	if low > high {
		return
	}
	A[low], A[low+(high-low)>>1] = A[low+(high-low)>>1], A[low]
	threshold := A[low]
	i, j := low, high
	for {
		if i >= j {
