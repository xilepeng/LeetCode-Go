...
func mergeSort(A []int, low, high int) {
	...
	mid := low + (high-low)>>1 // 分2部分定义当前数组
	mergeSort(A, low, mid)      // 排序数组的第1部分
	mergeSort(A, mid+1, high)   // 排序数组的第2部分
	merge(A, low, mid, high)    // 通过比较2个部分的元素来合并2个部分

}

// Your runtime beats 62.84 % of golang submissions
func merge(A []int, low, mid, high int) {
	tmpA := make([]int, high-low+1)
	i, j, k := low, mid+1, 0
	for p := low; p <= high; p++ {
	...
	copy(A[low:high+1], tmpA)
}



// Your runtime beats 58.68 % of golang submissions
