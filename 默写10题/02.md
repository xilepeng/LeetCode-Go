...
func isValid(s string) bool {
	...
	for _, ch := range s {
		if ch == '(' || ch == '[' || ch == '{' {
			...
			if top := stack[len(stack)-1]; top == '(' && ch == ')' ||
				top == '[' && ch == ']' || top == '{' && ch == '}' {
...
func lowestCommonAncestor(root, p, q *TreeNode) *TreeNode {
	...
	if root == p || root == q { // 递归出口
		return root
	} 
	left := lowestCommonAncestor(root.Left, p, q)   // 左子树是否包含 p、q
	right := lowestCommonAncestor(root.Right, p, q) // 右子树是否包含 p、q
	if left != nil && right != nil {                // 后续遍历
		return root
	}
	if left == nil {
		return right
